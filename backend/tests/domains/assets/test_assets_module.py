...
def test_ai_stub_with_feature_returns_501(
    ...
    assert_api_error(res, status=501, code="AI_SCHEDULING_NOT_AVAILABLE")

    get_settings.cache_clear()


def test_ai_stub_dev_env_returns_501(
    api_client, db_session, admin_user, site_with_asset, monkeypatch
):
    """In dev env (license bypassed), stub returns 501."""
    monkeypatch.setenv("APP_ENV", "development")
    from app.config import get_settings
